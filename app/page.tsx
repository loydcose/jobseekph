...
export default function Home() {
  ...
  return (
    <main className="bg-zinc-100 min-h-screen py-8 md:py-16">
      <section className="max-w-4xl mx-auto w-11/12">
        <div className="flex items-center gap-3 w-fit mx-auto mb-4 md:mb-8">
        ...
        <div className="grid grid-cols-[repeat(auto-fit,_minmax(250px,_1fr))] gap-4 mb-8">
          {jobs.map((job: Job, index) => (
            <Link
              ...
              className="animate__animated animate__zoomIn p-4 bg-white rounded-lg shadow-md hover:shadow-lg hover:scale-[101%] transition-all group flex flex-col"
            >
              ...
              <h2 className="mb-4 font-bold tracking-tight leading-5 text-zinc-800">
                {job.title}
              </h2>
              <p className="mb-1 text-sm text-zinc-500 mt-auto">{job.location}</p>
              <p className="font-bold text-sm leading-5 text-zinc-800">{job.salary}</p>
            </Link>
          ))}
        </div>
